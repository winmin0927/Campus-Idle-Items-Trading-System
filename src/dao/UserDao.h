...
public:
    ...
    long insert(const models::User& record) {
        auto result = dbClient->execSqlSync(
            "INSERT INTO user (accountNumber, userPassword, nickname, avatar, signInTime, userStatus) "
            "VALUES (?, ?, ?, ?, ?, ?)",
    ...
    std::optional<models::User> userLogin(const std::string& accountNumber, const std::string& userPassword) {
        auto result = dbClient->execSqlSync(
            "SELECT * FROM user WHERE accountNumber = ? AND userPassword = ?",
    ...
    std::vector<models::User> getNormalUser(int begin, int nums) {
        auto result = dbClient->execSqlSync(
            "SELECT * FROM user WHERE userStatus = 0 LIMIT ?, ?", begin, nums
        );
        return mapResultToList(result);
    }
    ...
    std::vector<models::User> getBanUser(int begin, int nums) {
        auto result = dbClient->execSqlSync(
            "SELECT * FROM user WHERE userStatus = 1 LIMIT ?, ?", begin, nums
        );
        return mapResultToList(result);
    }
    
    int countNormalUser() {
        auto result = dbClient->execSqlSync("SELECT COUNT(*) as count FROM user WHERE userStatus = 0");
        return result[0]["count"].as<int>();
    }
    
    int countBanUser() {
        auto result = dbClient->execSqlSync("SELECT COUNT(*) as count FROM user WHERE userStatus = 1");
        return result[0]["count"].as<int>();
    }
    
    bool updateByPrimaryKey(const models::User& record) {
        auto result = dbClient->execSqlSync(
            "UPDATE user SET accountNumber = ?, userPassword = ?, nickname = ?, "
            "avatar = ?, signInTime = ?, userStatus = ? WHERE id = ?",
            record.accountNumber, record.userPassword, record.nickname,
    ...
    bool updatePassword(const std::string& newPassword, const std::string& oldPassword, long id) {
        auto result = dbClient->execSqlSync(
            "UPDATE user SET userPassword = ? WHERE id = ? AND userPassword = ?",
...
private:
    ...
    models::User mapRowToUser(const Row& row) {
        ...
        user.accountNumber = row["accountNumber"].as<std::string>();
        user.userPassword = row["userPassword"].as<std::string>();
        user.nickname = row["nickname"].as<std::string>();
        user.avatar = row["avatar"].as<std::string>();
        user.signInTime = row["signInTime"].as<std::string>();
        user.userStatus = row["userStatus"].as<int>();
        return user;
    }
